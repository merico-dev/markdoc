...
该功能的主要目的在于支持潜在的数值替换需求，需要配合对应的解析代码使用，消费者请自行解析。
...
> 事实上，我们的文档服务器内置了对 `data-section` 属性的解析服务，只要在请求参数中设置了 `options.sections` 的属性，服务器在处理文档时便会返回对应的文档片段，具体使用方法见 API 文档。有一点需要注意的是，`data-section` 属性的值最好在同一文档内唯一，即便给多个文档片段设置了相同的 `data-section` 属性值，服务器也只会返回第一个出现 `data-section` 属性值的文档片段。
